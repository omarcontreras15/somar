...
class Venta extends Controller{
    ...
    public function cargarVistaVentasRealizadas(){
     ...
     public function cargarVistaVentasProgreso(){
         //varifica si hay o no una sesion iniciado, dependiendo del caso cargar el menuBar
        $menu=$this->cargarMenuBar();
        $ventana = $this->getTemplate("./app/views/ventas/ventasProgreso.html");
        ...
        $sizeArray = sizeof($array);
        $tablaVentas = "";
        $elementotabla = $this->getTemplate("./app/views/ventas/components/body-tabla-ventas-progreso.html");
        if($sizeArray>0){
            foreach ($array as $element){
                $temp = $elementotabla;
